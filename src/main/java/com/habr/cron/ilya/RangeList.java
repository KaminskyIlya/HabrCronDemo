...
final class RangeList implements Iterable<Range>
{
    ...
    /**
     * Calculates coverage by values in range.
     * if in range 0..59 only 3 values specified,
     * it return 0.05 (5% values coverage).
     *
     * @return 0..1
     */
    public float getCoverage()
    {
        int distance = getMaximum() - getMinimum() + 1;
        int values = 0;

        for (Range range : list)
        {
            int count = (range.max - range.min / range.step) + 1;
            values += count;
        }
